...
const AnimatedStack = (props) => {
  ...
  const rotate = useDerivedValue(
    () =>
      interpolate(translateX.value, [0, hiddenTransalteX], [0, ROTATION]) +
      "deg"
  );
  ...
  const likeStyle = useAnimatedStyle(() => ({
    opacity: interpolate(translateX.value, [0, hiddenTransalteX / 4], [0, 1]),
  }));
  ...
  const gestureHandler = useAnimatedGestureHandler({
    ...
    onEnd: (event) => {
      ...
      translateX.value = withSpring(
        hiddenTransalteX * Math.sign(event.velocityX),
        {},
